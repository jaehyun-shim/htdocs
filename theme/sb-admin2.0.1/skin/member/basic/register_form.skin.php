...
<div class="container">
		...
		<div class="card-body p-0">
			...
			<div class="row">
				...
				<div class="p-5">
					...
					<div class="form-group">
						<input type="text" name="mb_tel" value="<?php echo get_text($member['mb_tel']) ?>" id="reg_mb_tel" <?php echo $config['cf_req_tel']?"required":""; ?> class="form-control form-control-user <?php echo $config['cf_req_tel']?"required":""; ?>" maxlength="20" placeholder="전화번호">
					</div>
					<!--- ./form-group row --->
					<?php }  ?>
					...
					<div class="form-group">
						<input type="text" name="mb_hp" value="<?php echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" <?php echo ($config['cf_req_hp'])?"required":""; ?> class="form-control form-control-user <?php echo ($config['cf_req_hp'])?"required":""; ?>" maxlength="20" placeholder="휴대폰번호">
	...
	gif, jpg, png파일만 가능하며 용량 <?php echo number_format($config['cf_member_icon_size']); ?>바이트 이하만 등록됩니다.</span>
					...
					<div class="text-center">
						<button type="submit" id="btn_submit" class="btn_submit btn btn-primary btn-user btn-block" accesskey="s"><?php echo $w==''?'회원가입':'정보수정'; ?></button>
					</div>
